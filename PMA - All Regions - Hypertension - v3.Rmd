...
subtitle: Patient journey (Hypertension) - LNCC (Part 1/6)
...
This document includes preliminary journey of Hypertension patients analysis based on the Process Mining framework. We decided to analyze Prenatal, childbirth and Postpartum data once it is considered a bit more predictable in terms of journey than other clinical scenarios.  
In this sense, we included in this document some preliminary PMA graphics and tables aiming to reveal a first impression of the Hypertension patient journey in the São Paulo municipality for Prenatal, Childbirth and Postpartum activities. Also, we performed the same analysis to the five regions where the healthcare services are currently structured. Only data collected in 2015 were analyzed in this preliminary experiment.
...
IMPORTANT: in order to achieve better process map visualizations we are applying a percentile cut off of 85% of existing trace coverage. It means that the process will look at the 95% of the paths (set of steps) most performed by citizens during their journey in the public healthcare system.

```{r echo=FALSE}
# processando o diagrama de processo
a<- logevents %>%
    filter_trace_frequency(perc = 0.85)
rm(logevents)
```
...
a %>% process_map()
```
Figure 1 - Process map of Prenatal, Childbirth and Postpartum activities related to Hypertension patients performed in the Municipality of São Paulo in 2015 - All Regions
...
```
Figure 2 - Process map including absolute frequency and idle time of Prenatal, Childbirth and Postpartum activities related to Hypertension patients performed in the Municipality of São Paulo in 2015 - All Regions.  
...
a %>% process_map(type_nodes = frequency("relative"),
            type_edges = performance(median, "days"))
```
Figure 3 - Process map including relative frequency and idle time registered during the Prenatal, Childbirth and Postpartum activities related to Hypertension patients performed in the Municipality of São Paulo in 2015 - All Regions. 
...
```
Figure 4 - Summary of antecedent and consequent type of facility for the Prenatal, Childbirth and Postpartum activities related to Hypertension patients performed in the Municipality of São Paulo in 2015 - All Regions.
